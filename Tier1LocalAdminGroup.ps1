...
begin {
    ...
    $global:config = Get-JITconfig
    if ($global:config) {
        Write-LogFile -Severity Debug -Message "sucessfully read the JiT configuration"
    } else {
        Write-LogFile -Severity Error -Message "JiT configuration could not be loaded - aborting script!"
        $exit = $true
    }
...
process {
    ...
    #Region show progress activities
    #This section is not mandatory. It is only required for the interactive execution of the script to show the progress
    #Start time of the script to evaluate the runtime of the script
    $Starttime = Get-Date
    #Initialize the overall counter of detected computer object 
    $GroupCount = 0
    #Initialize the counter for executed groups activites.
    #Activities could be creating the group or remove permanent objects from the group
    $Statuscounter = 0 
    #endregion
    ...
    Foreach ($Domain in $aryDomainList) {
        ...
        Foreach ($SearchBase in $global:config.T1Searchbase) {
            ...
            if ($SearchBase -like "*$((Get-ADDomain -Server $Domain).DistinguishedName)") {
                ...
                #Display parameter to show the amount of computer object currently working on
                #when interactive execution
                $GroupCount += $T1ServerList.count 
    ...
    Remove-Variable -Name config -Scope Global -Force -ErrorAction SilentlyContinue
    ...
    Remove-Variable -Name Delegation -Force -ErrorAction SilentlyContinue
}
